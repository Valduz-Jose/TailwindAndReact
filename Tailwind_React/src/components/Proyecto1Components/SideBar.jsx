...
export const SideBar = () => {
    ...
    return (
        <section className=" top-0 left-0 h-screen bg-white dark:bg-[#1E1F22] shadow-lg  md:flex flex-col w-16 p-2">
            <SidebarIcon text="Home" funcion={()=>navigate(-1)} icon={<Icon icon="ic:baseline-discord" width="24" height="24"/>}/>
            <Divider/>
...
const SidebarIcon = ({icon,text="tooltip 🍕", funcion}) =>(
        <div onClick={funcion}
        className="relative flex items-center justify-center h-12 w-12 my-2 mx-auto bg-gary-400 dark:bg-[#313338] hover:bg-[#5865F2] text-white hover:text-white/80 rounded-full hover:rounded-xl transition-all duration-100 ease-linear cursor-pointer shadow-lg group">
            {icon}
...
const Divider = () =>(
<hr
        className="bg-gray-200 dark:bg-[#3a3a3a] border border-gray-200 dark:border-[#3a3a3a] rounded-full mx-2"
    />
);